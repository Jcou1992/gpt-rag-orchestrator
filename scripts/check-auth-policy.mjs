...
const FORBIDDEN_REGEX = [
  ...
  // ROUND-55 + ROUND-56: object-literal value alias. Matches:
  //   identifier key:    `{ ls: localStorage }`
  //   quoted-string key: `{ 'ls': localStorage }` / `{ "ls": ... }`
  //   computed key:      `{ [keyName]: localStorage }`
  // Each key shape is followed by `: localStorage` (optionally global-
  // qualified). The `[{,]` prefix anchors at the start of an object
  // literal entry.
  {
    multiline: true,
...
const FORBIDDEN_SCANNERS = [
  {
    ...
    scan(text) {
      const out = [];
      // ROUND-61 + ROUND-62: accept optional parens (with whitespace
      // BETWEEN each `(`) around the RHS qualified localStorage
      // expression. `(?:\(\s*)*` matches zero or more `(` each with
      // optional trailing whitespace, so `( (localStorage))` and
      // multi-line `(\n  (localStorage)\n)` shapes are caught. We
      // don't require matching `)` because the LHS walk only cares
      // about the `=` position.
